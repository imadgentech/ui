# @imadgentech/ui — Theme Reference

## Quick Start

Add to your Next.js app's `next.config.mjs`:
```js
transpilePackages: ['@imadgentech/ui']
```

Import the CSS tokens in your `globals.css`:
```css
...
// defaultTheme="dark" | "light", enableSystem
...
| Token                        | Dark             | Light    |
|------------------------------|------------------|----------|
| `--color-brand-primary`      | #ff6a00 (orange) | same     |
| `--color-brand-primary-hover`| #ff8a1f          | same     |
| `--color-brand-secondary`    | #ff8a1f          | same     |
| `--color-brand-accent`       | #ffae00          | same     |

Shorthand aliases (globals.css):
- `--orange` = #ff6a00
- `--orange2` = #ff8a1f
...
| `--shadow`    | 0 18px 50px rgba(0,0,0,.55) (site default)     | 0 18px 50px rgba(0,0,0,.12)                    |
...
### Heading Sizes (responsive / fluid)
| Token             | Range      |
|-------------------|------------|
| `--heading-sm`    | 14–16px    |
| `--heading-md`    | 18–24px    |
...
Site shorthand:
- `--h1Size` = clamp(42px, 6.2vw, 74px), weight 300, tracking -0.02em
- `--h2Size` = 28px, weight 300, tracking -0.02em
...
Default theme is **dark**. Switch themes by setting `data-theme="light"` on `<html>`.

The `Providers` component uses `next-themes` and handles this automatically.

All component styles reference the CSS variables above — no hardcoded colors.
